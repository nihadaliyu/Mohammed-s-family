...
PHOTO_DIRimport streamlit as st
import json
import os
import copy
import uuid
    ...
    with st.expander(f"{name} ({partner_display})", expanded=False):
        ...
        with card:
            ...
            with col2:
                ...
                # Single action spot: either Add Partner OR Add Child (same place)
                st.markdown('<div class="button-row">', unsafe_allow_html=True)
                show_add_partner = (not locked_root) and (not locked) and (not partner)
                show_add_child = (not locked_root) and (partner) and (not fixed)
                    ...
                    with st.form(f"form_partner_{key_base}"):
                        ...
                        if save_partner:
                            if pname.strip():
                                ...
                                # Close partner form; next render will show Add child button at same spot
                                st.session_state.pop(f"partner_mode_{key_base}", None)
                                save_and_rerun()
                            else:
