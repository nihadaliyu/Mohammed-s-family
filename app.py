...
def load_family_data():
    """
    Prefer reading DATA_FILE. If it exists and is valid JSON, load it.
    Otherwise return a deep copy of defaults (but DO NOT overwrite file).
    """
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            # Basic sanity: ensure mapping at top level
            if isinstance(data, dict):
                return data
        except Exception:
            # fall back to default copy
            pass
    # No valid file -> return default (don't write it automatically)
    return copy.deepcopy(default_family_data)

def save_family_data(data):
    # Save deterministic JSON using ensure_ascii=False so Amharic displays nicely if stored
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, ensure_ascii=False)
...
# ---------------- ADMIN CONFIG & SMART BROWSER CHECK ----------------
# allowed admin emails (edit to add real admin addresses)
...
# 1) If the app was redirected with a detected_email query param, use it (one-time)
# --- Modern Streamlit query param handling ---
qp = st.query_params
detected_email = None

if "detected_email" in qp:
    detected_email = qp["detected_email"].strip() if isinstance(qp["detected_email"], str) else str(qp["detected_email"])
    # clear query params to prevent re-trigger
    try:
    ...
    if detected_email:
        st.session_state.admin_email = detected_email
        if detected_email.lower() in {e.lower() for e in ADMIN_EMAILS}:
            st.session_state.is_admin = True
            st.session_state.user_role = "admin"
            st.session_state.quiz_done = True  # admins bypass quiz
        else:
            st.session_state.is_admin = False
            st.session_state.user_role = "guest"

# 2) Sidebar UI: let user choose Guest or Admin. When Admin logs in, save email to localStorage.
st.sidebar.markdown("### 🔐 መግቢያ")
role_choice = st.sidebar.radio("የተጠቃሚ ለውጥ", ("Guest (እንግዳ)", "Admin (አስዳኝ)"))

if role_choice == "Admin (አስዳኝ)":
    # show admin login input in sidebar
    email = st.sidebar.text_input("ኢሜይል ያስገቡ", value=st.session_state.get("admin_email", ""))
    if st.sidebar.button("Login (ግባ)"):
        email = email.strip()
        ...
        if email and email.lower() in {e.lower() for e in ADMIN_EMAILS}:
            ...
            st.session_state.user_role = "admin"
            st.session_state.quiz_done = True  # admins bypass quiz
            st.sidebar.success("እርስዎ አስዳኝ ናቸው።")
            # save this email to browser localStorage so future "Guest" selections can auto-detect it
            components.html(f"""
                <script>
                try {{
                  localStorage.setItem('saved_email', {json.dumps(email)});
                }} catch(e) {{ }}
                // no UI here; this frame will disappear
                </script>
            """, height=10)
        else:
            st.session_state.is_admin = False
            st.session_state.user_role = "guest"
            st.sidebar.error("ኢሜይል አልተፈቀደም — የተፈቀደ ኢሜይል ይጠቀሙ።")
else:
    # Guest selected: try to auto-detect a saved email in localStorage.
    # We'll inject a tiny HTML/JS that reads localStorage['saved_email'] and, if present,
    # redirects (replaceState) the browser URL adding a query param ?detected_email=... .
    # On reload Streamlit reads that query param above and elevates the session if it matches.
    st.session_state.user_role = "guest"

    # Only attempt detection when we don't already have a detected admin set in session
    # and only if we haven't tried in this session yet.
    if not st.session_state.get("is_admin", False) and not st.session_state.get("tried_local_email_detect", False):
        # Mark that we attempted detection to avoid repeating
        st.session_state.tried_local_email_detect = True
        components.html("""
        <script>
        (function() {
          try {
            ...
            if (saved) {
              // add detected_email query param if not present
              const url = new URL(window.location.href);
              const params = url.searchParams;
              if (!params.get('detected_email')) {
                params.set('detected_email', saved);
                // Use replaceState so it's a soft navigation (no reload) followed by a reload to pick up param.
                // We'll set location.href to the new URL to force Streamlit to receive the query param.
...
# small badge in sidebar about role
if st.session_state.is_admin:
    st.sidebar.success(f"Signed in as Admin: {st.session_state.admin_email}")
else:
    st.sidebar.info("Signed in as Guest (እንግዳ) — አንዳንድ ተግባራት የተከለከሉ ሊሆኑ ይችላሉ።")
...
def display_family(name, data, ancestors=None, level=0):
    ...
    # Determine partner display consistently from node["partner"]
    ...
    with st.expander(f"👤 {disp_name(name)} — {partner_display}", expanded=should_expand):
        ...
        with col1:
            ...
            st.image(show_img, width=100)
            if (not locked_root) and (not locked) and (not partner_live):
                if st.button("👫 ጋር ያቀርቡ", key=f"btn_partner_{key_base}"):
                    st.session_state[f"partner_mode_{key_base}"] = True
                    st.session_state.pop(f"child_mode_{key_base}", None)
                    st.session_state.pop(f"edit_mode_{key_base}", None)
            elif (not locked_root) and (partner_live) and (not fixed):
                if st.button("➕ ልጅ ያክሉ", key=f"btn_child_{key_base}"):
                    st.session_state[f"child_mode_{key_base}"] = True
                    st.session_state.pop(f"partner_mode_{key_base}", None)
                    st.session_state.pop(f"edit_mode_{key_base}", None)
        ...
        with col2:
            ...
            with desc_col:
                ...
                if st.button("📊 ሪፖርት አሳይ", key=f"rep_btn_{key_base}"):
                    st.session_state[f"show_report_{key_base}"] = not st.session_state.get(f"show_report_{key_base}", False)
                ...
                if st.session_state.get(f"show_report_{key_base}", False):
                    ...
                    st.markdown(f"""
                    <div class="report-box" style="margin-top: 5px;">
                        <div style="font-weight:700;color:#0b6cff;">📊 {disp_name(name)} የቤተሰብ ሪፖርት</div>
                        <div class="report-item">ልጆች: <b>{rep['gen2']}</b></div>
                        <div class="report-item">የልጆች ልጆች: <b>{rep['gen3']}</b></div>
                        <div class="report-item">ታላቅ የልጆች ልጆች: <b>{rep['gen4']}</b></div>
                        <div style="font-weight:700;margin-top: 4px;">ጠቅላላ የወረዱ ልጆች: <b>{rep['total_descendants']}</b></div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            with btn_col:
                if not locked_root:
                    if st.session_state.get("is_admin", False):
                        ...
                        # Guests may still edit local fields if desired — keep same behavior or disable; here we show button but warn on save
                        if st.button("✏️ አስተካክል", key=f"edit_{key_base}"):
                            st.session_state[f"edit_mode_{key_base}"] = True

                    if st.button("❌ ሰርዝ", key=f"del_{key_base}"):
                        if not st.session_state.get("is_admin", False):
                            st.error("ይቅርታ — ይህ እባክዎ አስዳኝ (Admin) የብቻ ነው። እባክዎ እንደ Admin ይግቡ።")
                        else:
                            if name in parent_children:
                                parent_children.pop(name, None)
                                save_and_rerun()
            ...
            if st.session_state.get(f"child_mode_{key_base}", False):
                with st.form(f"form_child_{key_base}"):
                    ...
                    cphone = st.text_input("ስልክ", key=f"cp_{key_base}")
                    cphoto = st.file_uploader("ፎቶ", type=["jpg", "jpeg", "png"], key=f"cph_{key_base}")
                    if st.form_submit_button("ልጅ አስቀምጥ"):
                        if not cname.strip():
                            st.error("ስም ያስፈልጋል።")
                        else:
                ...
                if st.session_state.get(f"edit_mode_{key_base}", False) and not locked_root:
                    with st.form(f"form_edit_{key_base}"):
                        ...
                        phone = st.text_input("ስልክ", value=node.get("phone", ""), key=f"ep_{key_base}")
                        if locked:
                            st.text_input("የባል/ሚስት ስም", value=node.get("partner", ""), disabled=True,
                                          key=f"pp_{key_base}")
                            pval = node.get("partner", "")
                        else:
                            pval = st.text_input("የባል/ሚስት ስም", value=node.get("partner", ""), key=f"epv_{key_base}")
                        photo = st.file_uploader("አዲስ ፎቶ", type=["jpg", "jpeg", "png"], key=f"eph_{key_base}")
                        save_edit = st.form_submit_button("አስቀምጥ")
                        cancel_edit = st.form_submit_button("ይቅር")
                        if cancel_edit:
                            st.session_state.pop(f"edit_mode_{key_base}", None)
                            st.rerun()
                        if save_edit:
                            parent = get_parent_container(ancestors)
                            # Guests can edit fields, but deletions/resets are admin-only (we already enforce delete/reset elsewhere)
                            if nname.strip() and (nname == name or nname not in parent):
                                node["description"] = desc
                                node["phone"] = phone
                                node["partner"] = pval
                ...
                # Reset (admin-only)
                if st.session_state.get("is_admin", False):
                    if st.button("🔄 ሁሉን ዳታ ዳግም (Admin)", key="reset_all"):
                        # preserve admin info across reset
                        admin_email = st.session_state.get("admin_email", "")
                        # replace family data with defaults
                    ...
                    if st.button("🔄 ሁሉን ዳታ ዳግም", key="reset_all_guest"):
                        st.error("ይህ እባክዎ ለአስዳኝ (Admin) ብቻ ነው። እባክዎ እንደ Admin ይግቡ።")
                ...
                if not st.session_state.get("is_admin", False):
                    ...
                    st.session_state.quiz_done = True  # ensure admin sees UI
                ...
                # Save button (always available; writes current session data to disk)
                if st.button("💾 ለውጦችን አስቀምጥ", key="save_changes"):
                    save_family_data(st.session_state.family_data)
                    st.success("ለውጦቹ በትክክል ተቀምጠዋል።")
